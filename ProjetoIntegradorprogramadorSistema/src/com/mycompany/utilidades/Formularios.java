/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package com.mycompany.utilidades;

import javax.swing.JFrame;

/**
 *
 * @author liberalino.1980
 */
public class Formularios {
    public static JFrame menu;
    
    public static JFrame login;
    
    public static JFrame botoesEscolhas;
    
    public static JFrame cadastroConteudo;
    
    public static JFrame tabelaConteudo;
    
    public static JFrame tabelaConteudoEditRemove;
    
    public static JFrame editModeConteudo;
    
    
    
    public static JFrame cadastroVendedor;
    
    public static JFrame tabelaVendedor;
    
    public static JFrame tabelaVendedorEditRemove;
    
    public static JFrame editModeVendedor;
    
    
    public static JFrame cadastroCliente;
    
    public static JFrame tabelaCliente;
    
    public static JFrame tabelaClienteEditRemove;
    
    public static JFrame editModeCliente;
}

